...
- Otherwise **let beads tell you what's next** — don't guess:
  ...
  - **A bead is ready** → name *that concrete bead* in a one-line seed prompt (it's self-contained: steps + acceptance live in its body):
    ...
    **Refresh-then-traverse.** A handoff means code changed, so the new session's index is stale — the SessionStart "Code Discovery Protocol" hook only indexes when a repo is *unindexed*, never refreshes a stale one. So the seed prompt above tells the fresh session to `detect_changes` and re-`index_repository` (only if stale) before traversing via `search_graph`/`trace_path`/`get_code_snippet`. Gate on availability — drop the clause if codebase-memory-mcp isn't in play. **Verbatim user prompts (above) stay verbatim — don't inject the refresh; the new session's normal indexing covers it.**
    ...
    <!-- autonomous-loop: fork-local -->
    **Before you arm `AUTONOMOUS-LOOP`, read the next bead's ACCEPTANCE — not just its title.** The stop conditions in `skills/_shared/autonomous-loop.md` are about the *task*, not only the moment you spawn: if the ready bead's own acceptance includes an outward-facing action, real spend, or a destructive/irreversible step (a production deploy, a data migration, a payment, sending mail, a `cdk deploy` against live infra), then arming an unattended loop on it means the loop will drive that step with no human in front of it. Don't. Either **omit** the `AUTONOMOUS-LOOP` marker (hand off as a single hop the human drives), or — if the earlier steps are safe and only the tail is not — **pin a stop-point `bd note` to the bead** ("AUTONOMOUS-LOOP STOP: implement + gate + PR autonomously, then STOP and ask before the deploy/publish/spend step") and say so in the seed. A loop that fans out into `deploy` because the bead said "deploy" is the same class of miss as filming the wrong claim: the marker delegated the *judgment*, but the judgment was the point.
...
- **An overstuffed seed that restates the bead.** The opposite failure from vagueness, and just as bad: cramming the bead's facts, traps, and verified findings into the prompt **forks the truth** — the same content now lives in two places, and the moment someone edits the bead the prompt is stale and lying. The seed is a *pointer plus at most the one trap not yet written down*. If you're writing more than a couple of sentences of context, that context belongs in a `bd note` on the bead (durable, the fresh session reads it on `bd show`), not in the ephemeral prompt. One line that says "claim `<id>`, read its body and notes first" beats a paragraph that duplicates them.
