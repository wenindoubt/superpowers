...
Under 100 chars, the very last line, nothing after it. The Stop hook reads this line to drive a hands-free loop: 🔴 stop (human decision), 🟡 keep this session working; 🟢 + ready beads + context ≥ the ctx gate `min(HANDOFF_CTX_PCT×window, HANDOFF_CTX_TOKENS)` (defaults 0.30 and 130k in `hooks/stop`; the absolute cap makes 1M windows actually trigger instead of parking at 300k) -> a fresh cmux session is spawned on the next `bd ready` bead (refreshing the codebase index first) and this session runs `/exit`. Soft-stall-as-🟢 means a non-blocking "what next?" advances the loop rather than waiting. Brakes: empty `bd ready`, 🟡/🔴, in-progress bead, `HANDOFF_DISABLE=1`.
<!-- /quick-recap: fork-local -->
